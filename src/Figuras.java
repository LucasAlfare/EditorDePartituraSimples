    ...
    private static void executarPath(String pathRelativo, Path2D painter, int x, int y, float escala){
        ...
        for (Conversor.OperacaoSVG pathAbsoluto : c.operacoesAbsolutas){
            switch (pathAbsoluto.comando.toLowerCase()){
                ...
                case "c":
                    painter.curveTo(
                            (pathAbsoluto.valores.get(0) + x) * escala, (pathAbsoluto.valores.get(1) + y) * escala,
                            (pathAbsoluto.valores.get(2) + x) * escala, (pathAbsoluto.valores.get(3) + y) * escala,
                            (pathAbsoluto.valores.get(4) + x) * escala, (pathAbsoluto.valores.get(5) + y) * escala
                    );
                    break;
                case "z":
                    painter.closePath();
